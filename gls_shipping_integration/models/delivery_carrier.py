...
class DeliveryCarrier(models.Model):
    ...
    gls_product_info = fields.Selection([('Parcel', 'PARCEL'),('Express', 'EXPRESS'),('Freight', 'FREIGHT')],
                                               string="GLS Product Info", help="The referenced product group must be available to the shipper that is referenced within the request")
    ...
    def gls_label_request_data(self,picking=False):
        ...
        etree.SubElement(consignee_address, "CountryCode").text = "%s" % (receiver_id.country_id and receiver_id.country_id.code or "")
        etree.SubElement(consignee_address, "Province").text = "%s" % (receiver_id.state_id and receiver_id.state_id.code or "")
        ...
        etree.SubElement(shipper_adress, "CountryCode").text = "%s" % (sender_id.country_id and sender_id.country_id.code or "")
        etree.SubElement(shipper_adress, "Province").text = "%s" % (sender_id.state_id and sender_id.state_id.code or "")
    ...
    def gls_send_shipping(self, pickings):
        ...
        for picking in pickings:
            try:
                ...
                data ="%s:%s"%(self.company_id and self.company_id.gls_user_id,self.company_id and self.company_id.gls_password)
                encode_data = base64.b64encode(data.encode("utf-8"))
                authrization_data = "Basic %s" % (encode_data.decode("utf-8"))
                url = "%s/backend/ShipmentProcessingService/ShipmentProcessingPortType"%(picking.company_id.gls_api_url)
                headers = {
                ...
                message = (_("Label created!<br/> <b>Label Tracking Number : </b>%s<br/> <b> Parcel Number : %s") % (track_id,parcel_number))
