...
class ProductProduct(models.Model):
    ...
    def data_generator(self):
        products = self.env['product.template'].search(
            [('is_onpage_product', '=', True), ('active', '=', True)]).mapped('product_variant_ids')
    ...
    def create_datas_list(self):
        ...
        for product in self.data_generator():
            entry_list = []
    ...
    def create_datas_list_qty(self, products, location_positions=False):
        products_list = []
        ...
        for product in products:
            ...
            if location_positions:
                for location in location_positions:
                    stock_qty = self.env['stock.quant'].search([('product_id', '=', product.id),
                                                                ('location_id', '=', location.id)])
                    if stock_qty:
                        qty_sum = 0
                        for qty in stock_qty:
                            qty_sum += qty.quantity
                        entry_list.append(str(qty_sum))
                    else:
                        entry_list.append("")
            else:
                if product.free_qty:
    ...
    def _create_csv_file(self):
        ...
        datas_list = self.create_datas_list()
    ...
    # Nuova funzione per inviare solamente SKU e Giacenza
    def _create_csv_file_quantity(self):
        trigger_user_id = self.env.user.id
        config_sudo = self.env['ir.config_parameter'].sudo()
        prod_ids = self.mapped('id')
        if prod_ids:
            products = self.env['product.product'].search([('id', 'in', prod_ids), ('active', '=', True)])
        else:
            products = self.env['product.product'].search([('onpage_variant_id', '!=', False), ('active', '=', True)])
        multiple_location = config_sudo.get_param('single_stock')
        location_ids = self.env['stock.location'].search([('is_onpage', '=', True)], order='id asc')
        location_positions = []
        if not multiple_location:
            headers_list = [
                'SKU', 'Giacenza'
            ]
        else:
            ...
            for location in location_ids:
                headers_list.append(location.onpage_str)
                location_positions.append(location)
        file_name_model = 'Onpage_product_qty_export_' + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".csv"
        path = config_sudo.get_param('path_onpage_attachments')
        onpage_token_qty = config_sudo.get_param('onpage_token_qty')
        if not path:
            raise UserError(
                "Attenzione! Il path per gli allegati non è stato configurato. Si prega di contattare l'amministratore.")
        if not onpage_token_qty:
            raise UserError(
                "Attenzione! Il token per l'invio dati a Onpage non è stato configurato. Si prega di contattare l'amministratore.")
        if path[-1] != '/':
            file_name_path = '/' + file_name_model
        ...
        writer = csv.writer(file)
        writer.writerow(headers_list)
        datas_list = self.create_datas_list_qty(products, location_positions) if multiple_location else self.create_datas_list_qty(products)
        for line in self.list_generator(datas_list):
            writer.writerow(line)
        file.close()
        ...
        # Invio a onpage tramite API
        try:
            url = f"https://app.onpage.it/api/import/with-config/{onpage_token_qty}"
            res = requests.post(url, files={
                'file': open(file_name),
            })
            if res.status_code != 200:
            ...
            _logger.error("L'invio del file csv qty non è riuscito correttamente :%s", e)
            outcome = 'error_odoo'
            error_message = e
    ...
    def temporary_onpage_action(self):
        product_templates = self.env['product.product'].search(
            [('onpage_variant_id', '!=', False), ('active', '=', True),
             ('collection_id', '!=', False),
             ('collection_id.brand_id', '!=', False)]).mapped('product_tmpl_id')
        for template in product_templates:
            template.is_onpage_product = True