...
class KsWooImageHandler(http.Controller):
    ...
    @http.route(['/ks_woo_image/variant/<string:db>/<int:uid>/<int:product_product_id>/<int:product_image_id>/<string:image>'], auth='none',
                csrf=False, methods=['GET', 'POST'], type='http')
    def create_woo_public_product_variant_image_url(self, db=False, uid=False, product_product_id=False, product_image_id=False, image=False, **post):
        db = db.strip()
        request.session.db = db
        request.session.uid = uid
        request.uid = uid
