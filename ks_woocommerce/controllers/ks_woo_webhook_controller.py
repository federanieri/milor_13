...
class KsWooWebhookHandler(http.Controller):
    ...
    @http.route(['/woo_hook/<int:woo_instance_id>/<string:db>/<int:uid>/order/create'], auth='none', csrf=False, methods=['POST'])
    ...
    @http.route(['/woo_hook/<int:woo_instance_id>/<string:db>/<int:uid>/order/update'], auth='none', csrf=False, methods=['POST'])
        ...
        if request.httprequest.data:
            ...
            if woo_instance_id:
                ...
                if woo_instance and data:
                    wcapi = woo_instance.ks_api_authentication()
                    if wcapi.get('').status_code in [200, 201]:
                        request.env['sale.order'].sudo().with_context({'uid': SUPERUSER_ID}).ks_manage_sale_order_data(
                            data, wcapi, woo_instance)
                    else:
                        request.env['ks.woo.sync.log'].create_log_param(ks_woo_id=False,
                                                                        ks_status='success' if wcapi.get(
                                                                            "").status_code in [200,
