...
class KsWooProductVariantImage(models.Model):
    ...
    def ks_variant_woo_image_url(self, template_id, image_id, filename):
        ...
        base_url = self.ks_woo_instance_id.ks_base_url
...
class KsWooProductVariant(models.Model):
    ...
    ks_variant_profile_image = fields.Image()
    ...
    def change_regular_price_in_pricelist(self):
        ...
        for rec in self:
            if rec.ks_woo_variant_reg_price >= rec.ks_woo_variant_sale_price:
                self.ks_woo_instance_id.ks_woo_pricelist.item_ids.search(
                    [('pricelist_id', '=', rec.ks_woo_instance_id.ks_woo_pricelist.id),
                     ('applied_on', '=', '0_product_variant'),
                     ('product_id', '=', rec.id),
                     ('compute_price', '=', 'fixed'),
                     ('ks_instance_id', '=', rec.ks_woo_instance_id.id)],
                    limit=1).fixed_price = rec.ks_woo_variant_reg_price
            else:
            ...
            if rec.ks_woo_variant_sale_price <= rec.ks_woo_variant_reg_price:
                rec.ks_woo_instance_id.ks_woo_sale_pricelist.item_ids.search(
                    [('pricelist_id', '=', rec.ks_woo_instance_id.ks_woo_sale_pricelist.id),
                     ...
                     ('compute_price', '=', 'fixed'),
                     ('ks_instance_id', '=', rec.ks_woo_instance_id.id)],
                    limit=1).fixed_price = rec.ks_woo_variant_sale_price
            else:
        ...
        for rec in self:
            if not rec.ks_file_type == rec.ks_product_image_id.ks_file_name:
                data = self.env['ks.woo.product.variant.image'].create({
                    ...
                    'ks_product_variant_id': rec.ids[0],
                })
    ...
    # This function updates the price of the product variant on the pricelist
    def ks_update_price_on_product_variant(self, json_data):
        """
        :param json_data: product variant data from the woocommerce
        :return: none
        """
        pricelists = self.ks_woo_instance_id.ks_woo_sale_pricelist.search(
            [('ks_instance_id', '=', self.ks_woo_instance_id.id), ('ks_onsale_pricelist', '=', True), ('currency_id', '=', self.ks_woo_instance_id.ks_woo_currency.id),])
        if not float(json_data.get('price') or 0.0) == float(json_data.get('sale_price') or 0.0):
            self.ks_woo_variant_reg_price = float(json_data.get('regular_price') or 0.0)
            self.ks_woo_variant_sale_price = 0.0
        else:
            woo_variant_sale_price = float(
            json_data.get('sale_price') or self.ks_woo_instance_id.ks_woo_sale_pricelist.search(
            ...
            self.ks_woo_variant_reg_price = float(json_data.get('regular_price') or 0.0)
            if self.ks_woo_variant_reg_price < woo_variant_sale_price:
                self.ks_woo_variant_reg_price = float(woo_variant_sale_price or 0.0)
            self.ks_woo_variant_sale_price = float(
            json_data.get('sale_price') or self.ks_woo_instance_id.ks_woo_sale_pricelist.search(
                [('ks_instance_id', '=', self.ks_woo_instance_id.id),
                 ('currency_id', '=', self.ks_woo_instance_id.ks_woo_currency.id),
                 ('ks_onsale_pricelist', '=', True)]).item_ids.search([('pricelist_id', '=', pricelists.id),
