...
class KsSaleOrderInherit(models.Model):
    ...
    def ks_manage_sale_order_data(self, json_data, wcapi, instance_id):
        ...
        if not sale_order_exist.ks_date_updated == datetime.datetime.strptime(json_data.get('date_modified'),'%Y-%m-%dT%H:%M:%S'):
    ...
    def prepare_sale_order_data_for_odoo(self, each_record, wcapi, instance_id):
        if instance_id.ks_multi_currency_option:
            currency_id = self.env['res.currency'].search([('name', '=', each_record.get('currency'))])
        else:
    ...
    def _get_order_lines(self, json_data, wcapi, instance_id, pricelist_id):
        ...
        for each_record in json_data.get('line_items'):
            ...
            line_items_data = {
                ...
                # 'price_unit': each_record.get('price'),
                'ks_discount_amount': self.ks_calculate_percent_from_amont(float(ks_price_unit*each_record.get('quantity') or 0),
        ...
        for each_rec in json_data.get('shipping_lines'):
            ...
            #     shipping_price = float(each_rec.get('total') or 0)
            # elif tax_value == 'tax_included':
            #     shipping_price = float(each_rec.get('total') or 0) + float(each_rec.get('total_tax') or 0)
            # else:
            #     shipping_price = float(each_rec.get('total') or 0)
            shipping_lines_data = {
                'ks_woo_id': each_rec.get('id'),
                ...
                'tax_id': [(6, 0, self.with_context({'shipping_data': True}).get_tax_ids(json_data.get('tax_lines'),
                                                                                         each_rec.get('taxes'),
                                                                                         instance_id, wcapi))]
            }
            if sale_order_line_exist:
...
class KsSaleOrderLineInherit(models.Model):
    ...
    def _prepare_invoice_line(self):
        ...
        res = super(KsSaleOrderLineInherit, self)._prepare_invoice_line()
        if self.ks_discount_amount and (self.qty_to_invoice * self.price_unit):
            discount = (self.ks_discount_amount / (self.qty_to_invoice * self.price_unit) if (self.qty_to_invoice * self.price_unit) > 0 else 1) * 100
