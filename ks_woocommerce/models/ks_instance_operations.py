...
class KsWooInstanceOperation(models.TransientModel):
    ...
    ks_date_filter_info = fields.Char(default="[Date filter works on Products, Sale Orders and Customers.]",
                                      readonly=True)
    #Todo
    ks_sync_images_perm_ = fields.Boolean()
    ks_sync_products_with_images = fields.Boolean('Sync Products with Images')
    ...
    def ks_execute_operation(self):
        ...
        if self.ks_date_filter:
            if not self.ks_date_from:
                raise ValidationError(_('Please select start date for filter'))
            if not self.ks_date_to:
                raise ValidationError(_('Please select end date for filter'))
        ...
        if self.ks_sync_orders or self.ks_sync_customers or self.ks_sync_coupons or self.ks_sync_products or \
                ...
                self.ks_update_customers or self.ks_update_products or self.ks_update_products_with_images or self.ks_update_coupons or \
                self.ks_update_attributes or self.ks_update_category or self.ks_update_tags or \
                self.ks_update_order_status or self.ks_update_stock or self.ks_import_stock or \
                self.ks_sync_products_with_images:
                ...
                if each_instance.ks_instance_state == 'active':
                    try:
                        ...
                        if wcapi.get("").status_code in [200, 201]:
                            ...
                            # used when sync_products_with_images
                            # if self.ks_sync_products_with_images:
                            #     _logger.info('Product Syncing start For WooCommerce Instance [%s -(%s)]'
                            #                  , each_instance.ks_name, each_instance.id)
                            #     self.env['ks.woo.queue.jobs'].with_context({'ks_date_filter': self.ks_date_filter,
                            #                                                 'ks_date_from': self.ks_date_from,
                            #                                                 'ks_date_to': self.ks_date_to,
                            #                                                 'sync_products_with_images': True}).ks_sync_product_woocommerce_in_queue(
                            #         wcapi=wcapi, instance_id=each_instance)
                            if self.ks_sync_customers:
                                ...
                                self.env['ks.woo.queue.jobs'].with_context({'ks_date_filter': self.ks_date_filter,
                                                                            'ks_date_from': self.ks_date_from,
                                                                            'ks_date_to': self.ks_date_to}).ks_sync_customer_woocommerce_in_queue(
                                    wcapi=wcapi, instance_id=each_instance)
                            ...
                            if self.ks_sync_orders:
                                ...
                                self.env['ks.woo.queue.jobs'].with_context({'ks_date_filter': self.ks_date_filter,
                                                                            'ks_date_from': self.ks_date_from,
                                                                            'ks_date_to': self.ks_date_to}).ks_sync_sale_order_to_queue(
                                    wcapi=wcapi, instance_id=each_instance)
                                    ...
                                    if len(product_records_data['update']) >= 100:
                                        ...
                                        for each_rec in range(0, no_of_batches):
                                            batch_product_records = product_records_data['update'][
                                                                    0 + (100 * each_rec):100 + (100 * each_rec)]
                            ...
                            if self.ks_update_products:
                                ...
                                self.env['ks.woo.queue.jobs'].with_context(
                                    {'ks_image_permission': False}).ks_update_product_to_queue(products_records,
                                                                                               each_instance)
                            if self.ks_update_products_with_images:
                                _logger.info('Updating the products on WooCommerce Instance [%s -(%s)]'
                                             , each_instance.ks_name, each_instance.id)
                                products_records = self.env['product.template'].search(
                                    [('ks_woo_instance_id', '=', each_instance.id), ('ks_to_be_export', '!=', False)])
                                self.env['ks.woo.queue.jobs'].with_context(
                                    {'ks_image_permission': True}).ks_update_product_to_queue(products_records,
                                                                                              each_instance)
                            ...
                            if self.ks_update_attributes:
                                ...
                                self.env['ks.woo.queue.jobs'].ks_update_product_attribute_to_queue(attributes_records,
                                                                                                   each_instance)
                            if self.ks_update_tags:
                                ...
                                tags_records = self.env['ks.woo.product.tag'].search(
                                    [('ks_woo_instance_id', '=', each_instance.id)])
                                self.env['ks.woo.queue.jobs'].ks_update_product_tag_to_queue(tags_records,
                                                                                             each_instance)
                            if self.ks_update_category:
                                ...
                                category_records = self.env['product.category'].search(
                                    [('ks_woo_instance_id', '=', each_instance.id)])
                                self.env['ks.woo.queue.jobs'].ks_update_product_category_to_queue(category_records,
                                                                                                  each_instance)
                            ...
                            if self.ks_update_customers:
                                ...
                                customer_records = self.env['res.partner'].search(
                                    [('ks_woo_instance_id', '=', each_instance.id)])
                                self.env['ks.woo.queue.jobs'].ks_update_customer_woocommerce_in_queue(customer_records,
                                                                                                      each_instance)
