...
class KsPartnerInherit(models.Model):
    ...
    @api.model
    def ks_import_individual_customer_to_odoo(self):
        """
    ...
    def ks_import_individual_customer_to_odoo_wizard(self):
        ...
        for rec in self:
            ks_failed_list = rec.ks_import_individual_customer_to_odoo()
        ...
        # for rec in self:
        #     rec.ks_import_individual_customer_to_odoo()
        # return self.env['ks.message.wizard'].ks_pop_up_message(names='Success',
        #                                                        message='Import Operation has been performed, '
        #                                                                'Please refer logs for further details.')
                    ...
                    if wcapi.get('').status_code in [200, 201]:
                        if record.ks_woo_id:
                            ...
                            if woo_customer_response.status_code == 404:
                                ...
                                record = self.ks_update_customer_on_woo(wcapi, data, record)
                                ks_list = record
                        else:
                            record = self.ks_create_customer_on_woo(wcapi, data, record)
                            ks_list = record
