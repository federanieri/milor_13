...
class KsAccountPaymentInherit(models.Model):
    ...
    def post(self):
        ...
        if self.env['ks.woocommerce.instances'].search([('id', '=', instance_id)]).ks_woo_auto_order_status:
            self.env['sale.order'].search([('id', '=', self.ks_woo_sale_order_id.id)]).ks_woo_status = self.env[
                'ks.woocommerce.instances'].search([('id', '=', instance_id)]).ks_woo_order_invoice_selection
...
class KsAccountInvoiceInherit(models.Model):
    ...
    def refund_in_woo(self):
        ks_instance_id = self.env['ks.woocommerce.instances'].search(
            [('id', '=', self.ks_woo_order_id.ks_woo_instance_id.id)])
    ...
    def button_validate(self):
        ...
        if ks_instance_id.ks_woo_auto_order_status:
            self.env['sale.order'].search(
                [('id', '=', self.sale_id.id)]).ks_woo_status = ks_instance_id.ks_woo_order_shipment_selection
        return res
