...
class KsProductPricelistInherit(models.Model):
    ...
    ks_onsale_pricelist = fields.Boolean('OnSale Pricelist', default=False, readonly=True)
...
class KsProductPricelistItemInherit(models.Model):
    ...
    def ks_onchange_fixed_price(self):
        """
        :return: none
        """
        if self.product_id.ks_woo_product_type == 'variable':
            ...
            ks_woo_regular_price = self.product_id.ks_woo_regular_price
            ks_woo_sale_price = self.product_id.ks_woo_sale_price
        if self.ks_on_sale_price:
            if ks_woo_regular_price >= self.fixed_price:
                if self.product_id.ks_woo_product_type == 'variable':
                    self.product_id.ks_woo_variant_sale_price = self.fixed_price
                else:
                    self.product_id.ks_woo_sale_price = self.fixed_price
            else:
            ...
            if self.fixed_price != 0 and not self.product_id.ks_woo_product_type == 'variable':
                self.product_id.list_price = self.fixed_price
        elif not self.ks_on_sale_price:
            if ks_woo_sale_price <= self.fixed_price:
                if self.product_id.ks_woo_product_type == 'variable':
                    self.product_id.ks_woo_variant_reg_price = self.fixed_price
                else:
                    self.product_id.ks_woo_regular_price = self.fixed_price
            else:
                raise ValidationError("Regular price cannot be less than Sale Price !")
            if self.product_id.ks_woo_sale_price == 0 and not self.product_id.ks_woo_product_type == 'variable':
                self.product_id.list_price = self.fixed_price