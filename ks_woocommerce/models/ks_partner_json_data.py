                ...
                if not address_data:
                    ...
                    for each_add in addresses:
                        if not new_address:
                            ...
                            woo_child_add = {}
                            woo_child_add.update(json_data.get(i))
                            woo_name = json_data.get(i).get('name').strip()
                            woo_child_add.update({'name': woo_name})
                            new_address = self._match_address(address, woo_child_add)
                            if new_address:
                    ...
                    if not new_address:
                        ...
                        # if addresses and self.ks_woo_id != 0:
                        #     address_ = addresses[0].write(updated_address)
    ...
    def _match_address(self, address1, address2):
        if address1 and address2:
            if {c: address1[c] for c in address1 if c in address2 and address1[c] != address2[c]}:
                for key, value in {c: address1[c] for c in address1 if c in address2 and address1[c] != address2[c]}.items():
                    if value:
                        return False
        return True
