...
class DeliveryCarrier(models.Model):
    ...
    delivery_type = fields.Selection(selection_add=[("gls_italy", "GLS Italy")])
    ...
    def gls_itlay_request_quotation_request_data(self, order):
        ...
        etree.SubElement(Quotation, 'RagioneSociale').text = "{}".format(
            receiver_id.name if receiver_id.company_type == 'company' else receiver_id.parent_id.name)
    ...
    def gls_itlay_create_order_request_data(self, pickings):
        ...
        if receiver_id.phone:
            receiver_phone = "{}".format(receiver_id.phone.replace(' ','')) if '+' in receiver_id.phone else "+{}".format(receiver_id.phone.replace(' ','') or '')
        else:
            receiver_phone = ""
        ...
        if pickings.package_ids:
            for package_data in pickings.package_ids:
                ...
                etree.SubElement(parcel_data, 'Bda').text = "{0}/{1}".format(pickings.origin,package_data.name[6:-1]) if pickings.origin else "{}".format(package_data.name)
                ...
                etree.SubElement(parcel_data, 'Cellulare1').text = receiver_phone
                ...
                etree.SubElement(parcel_data, 'TelefonoDestinatario').text = receiver_phone
        ...
        if bulk_weight:
            ...
            etree.SubElement(parcel_data, 'Zipcode').text = "{}".format(receiver_id.zip)
            etree.SubElement(parcel_data, 'Provincia').text = "{}".format(receiver_id.country_id.code)
            etree.SubElement(parcel_data, 'Bda').text = "{0}/{1}".format(pickings.name.replace('/',''),pickings.origin) if pickings.origin else "{}".format(pickings.name.replace('/',''),pickings.origin)
            etree.SubElement(parcel_data, 'Colli').text = "1"
        ...
        try:
            ...
            if response_data.status_code in [200, 201]:
                ...
                for parcel_data in parcel_response:
                    ...
                    if (not shipping_number or shipping_number == None) or (not label_data or label_data == None):
                        raise ValidationError(_('Tracking number or Label data not found in GLS Response \n Response '
                                                'data {}').format(result))
                ...
                shipping_data = {
                    'exact_price': 0.0,
                    'tracking_number': ",".join(set(shipping_number_lst))}
                response = []
    ...
    def gls_italy_get_tracking_link(self, pickings):
        ...
        for picking in pickings:
            if 'BAT/OUT' in picking.name:
                url = "https://www.gls-italy.com/index.php?option=com_gls&amp;view=track_e_trace&amp;mode=search&amp;diretto=yes&amp;locpartenza=V6&amp;numsped=%s" % pickings.carrier_tracking_ref
                break
            else:
                url = "https://www.gls-italy.com/index.php?option=com_gls&amp;view=track_e_trace&amp;mode=search&amp;diretto=yes&amp;locpartenza=M4&amp;numsped=%s" % picking.carrier_tracking_ref
                break
        return url
