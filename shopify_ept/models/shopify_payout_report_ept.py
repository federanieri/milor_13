...
class ShopifyPaymentReportEpt(models.Model):
    ...
    def get_payout_report(self, start_date, end_date, instance):
        ...
        @author: Meera Sidapara on Date 04-Mar-2022.
        """
    ...
    def shopify_list_all_transactions(self, result):
        """
           This method used to call the page wise data import for payout report transaction from Shopify to Odoo.
           @param : result
           @author: Meera Sidapara @Emipro Technologies Pvt. Ltd on date 04/03/2022.
       """
        transactions_list = []
        catch = ""
        while result:
            ...
            if transaction.transaction_type in ['charge', 'refund', 'payment_refund'] and not order_id:
                ...
                if order_id:
                    ...
                    message = "Transaction line {0} will not automatically reconcile due to " \
                              "order {1} is not found in odoo.".format(transaction.transaction_id,
                    ...
                    payout_logline_obj.create({'message': message,
                                               ...
                                               'is_skipped': True,
                                               'payout_id': self.id})
                    bank_line_vals = self.prepare_bank_statement_line_vals(transaction, reference, partner,
                                                                           bank_statement_id)
                    bank_statement_line_obj.create(bank_line_vals)
    ...
    #     if len(transaction_ids) == 250:
    #         transaction_ids = self.shopify_list_all_transactions(transaction_ids, response, url)
