...
class SaleOrder(models.Model):
    ...
    def prepare_vals_shopify_multi_payment_refund(self, order_refunds, order):
        """ This method is used to manage multi payment wise remaining refund amount.
            ...
            @return: True
            @author: Meera Sidapara @Emipro Technologies Pvt. Ltd on date 1 March 2022.
            Task_id: 185069
        """
    ...
    def shopify_create_order(self, instance, partner, shipping_address, invoice_address,
        ...
        # Nexapp ~ Obtain Gateway
        payment_gateway_names = order_response.get('payment_gateway_names', [])
        gateway = payment_gateway_names[0] if payment_gateway_names else "no_payment_gateway"
        if not gateway:
        ...
        self.set_fulfilment_order_id_and_fulfillment_line_id(order)
        return order

    def set_fulfilment_order_id_and_fulfillment_line_id(self, order):
        """
        This method sets order line fulfillment order id and fulfillment line item id
        based on Shopify fulfillment order response.
        @author:Meera Sidapara @Emipro Technologies Pvt. Ltd on date 23 March 2023.
        Task Id : 221808 - Shopify Library changes in v13
        """
    ...
    def prepare_vals_shopify_multi_payment(self, instance, order_data_queue_line, order_response, log_book_id,
        ...
        for transaction in transactions:
            ...
            if result.get('kind') in ['capture', 'sale'] and result.get('status') == 'success':
                ...
                # Nexapp ~ Obtain Gateway
                payment_gateway_names = order_response.get('payment_gateway_names', [])
                order_gateway = payment_gateway_names[0] if payment_gateway_names else ""
    ...
    def update_order_status_in_shopify(self, instance):
        ...
        for picking_id in picking_ids:
            ...
            try:
                self.set_fulfilment_order_id_and_fulfillment_line_id(sale_order)
                order = shopify.Order.find(sale_order.shopify_order_id)
            ...
            if order_lines and order_lines.filtered(
                    lambda s: s.product_id.categ_id.id == 1 and s.product_id.type != 'service' and s.shopify_fulfillment_line_id == False or ''):
                message = (_(
            ...
            if not manage_multi_tracking_number_in_shopify_delivery_order:
                ...
                for move in not_kit_moves:
                    if move.sale_line_id and move.sale_line_id.shopify_fulfillment_line_id:
                        shopify_fulfillment_line_id = move.sale_line_id.shopify_fulfillment_line_id
                    line_items.update({shopify_fulfillment_line_id: {'tracking_no': [picking.carrier_tracking_ref],
                ...
                for kit_sale_line in kit_sale_line_ids:
                    if kit_sale_line and kit_sale_line.shopify_fulfillment_line_id:
                        shopify_fulfillment_line_id = kit_sale_line.shopify_fulfillment_line_id
                    line_items.update({shopify_fulfillment_line_id: {'tracking_no': [picking.carrier_tracking_ref],
                ...
                for move in not_kit_moves:
                    ...
                    for tracking_no, qty in tracking_number_wise_details.items():
                        ...
                        if move.sale_line_id and move.sale_line_id.shopify_fulfillment_line_id:
                            shopify_fulfillment_line_id = move.sale_line_id.shopify_fulfillment_line_id
                        line_items.update({shopify_fulfillment_line_id: {'tracking_no': [tracking_no],
                                                                         'qty': qty,
                                                                         }})
            ...
            for sale_line_id in line_items:
                ...
                update_lines.append({'id': sale_line_id,
                                     'quantity': int(line_items.get(sale_line_id).get('qty'))})
            for line in sale_order_line_ids:
                quantity = sum(
                    sale_order.order_line.filtered(lambda l: l.shopify_fulfillment_line_id == line).mapped(
                        'product_uom_qty'))
            ...
            fulfillment_vals = self.prepare_vals_for_fulfillment(sale_order, shopify_location_id, tracking_numbers,
                                                                 picking, carrier_name, update_lines, notify_customer)
    ...
    def prepare_vals_for_fulfillment(self, sale_order, shopify_location_id, tracking_numbers, picking, carrier_name,
                                     update_lines, notify_customer):
        """ This method is used to prepare a vals for the fulfillment.
            @return: fulfillment_vals
            @author: Meera Sidapara @Emipro Technologies Pvt. Ltd on date 23/03/2023.
            Task_id: 221808
        """
        tracking_info = {}
        if carrier_name:
            ...
            "line_items_by_fulfillment_order": [
                {
                    ...
                    "fulfillment_order_line_items": update_lines
                }]
        }
        if tracking_info:
        ...
        if self.amount_total == total_refund and not refunds:
            move_reversal = self.env["account.move.reversal"].with_context(
                ...
                {"refund_method": "cancel",
                 ...
                 "date": refund_date})
            new_move = self.env['account.move'].browse(move_reversal.reverse_moves()['res_id'])
            new_move.write({'is_refund_in_shopify': True, 'shopify_refund_id': refunds_data[0].get('id')})
            return [True]
        return [3]
    ...
    def create_move_and_delete_not_necessary_line(self, refunds_data, invoices):
        ...
        move_reversal = self.env["account.move.reversal"].with_context(
            ...
            refunds_data.get("note"), "date": refund_date})
