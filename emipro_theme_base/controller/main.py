...
class EmiproThemeBase(http.Controller):
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, product_custom_attribute_values = None, **kw):
        ...
        if sale_order.partner_id.parent_id and not sale_order.order_line:
            addresses = sale_order.partner_id.address_get(['delivery', 'invoice'])
            sale_order.partner_shipping_id = addresses['delivery']
            sale_order.partner_invoice_id = addresses['invoice']
    ...
    def Brand(self, brand=None, page=0, category=None, search='', ppg=False, **post):
        ...
        domain = WebsiteSaleFilter._get_search_domain(self, search, category, attrib_values,filter_values=filter_values)
        ...
        search_product = Product.with_context(lang=u'en_US').search(domain)
        ...
        products = Product.with_context(lang=u'en_US').search(domain, limit=ppg, offset=pager['offset'],
